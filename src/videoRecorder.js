...
function getMediaStreams() {
  let canvasUnity = document.querySelector("#unity-canvas");
...
function initAudioStream() {
  ...
  var listSourceNote = {};
  listLink.forEach((link, index) => {
    listSourceNote[`sourceNode${index}`] = audioCtx.createMediaElementSource(
      listAudioElement[index]
    );
...
function mediaStart() {
  ...
  mediaRecorder.onstop = function (e) {
    ...
    //video.src = videoURL;
    if (typeof window.NativeSendBlobUrlAndBlob !== "undefined") {
      window.NativeSendBlobUrlAndBlob(videoURL, blob);
    } //, isShow); }
    else {
      downloadVideo();
    }
...
function initAudioPlayPause() {
  listLink.forEach((link, index) => {
    // start the video, pause output to our headphones
...
function downloadVideo() {
  ...
  switch (containerType) {
    case "video/mp4":
      var name = "video-recording.mp4";
      break;
    default:
      var name = "video-recording.webm";
      break;
  }
  link.download = name;
  link.href = videoURL;
...
document.addEventListener("visibilitychange", (event) => {
  if (document.visibilityState === "visible") {
    ...
    if (listAudioElement.length > 0) {
      //console.log("listAudioElement is set");
      if (listAudioElement[0].paused) {
        listAudioElement[0].play();
      } //console.log("listAudioElement[0] is paused");
    }
  } else {
    //console.log("tab is inactive")
    if (listAudioElement.length > 0) {
      for (let i = 0; i < listAudioElement.length; i++) {
        listAudioElement[i].pause();
      }
    }
  }
});
